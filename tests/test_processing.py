from typing import Any

import pytest

from src.processing import filter_by_state, sort_by_date


@pytest.mark.parametrize(
    "start_list, state_id",
    [
        ([{"id": 41428829, "state": "EXECUTED", "date": "2019-07-03T18:35:29.512364"}], "EXECUTED"),
        ([{"id": 939719570, "state": "EXECUTED", "date": "2018-06-30T02:08:58.425572"}], "EXECUTED"),
    ],
)
def test_filter_by_state(start_list: Any, state_id: Any) -> Any:
    try:
        assert filter_by_state(start_list) == state_id
    except AssertionError:
        print("Некорректные данные")


@pytest.mark.parametrize(
    "start_list, revers",
    [
        ([{"id": 41428829, "state": "EXECUTED", "date": "2019-07-03T18:35:29.512364"}], "EXECUTED"),
        ([{"id": 939719570, "state": "EXECUTED", "date": "2018-06-30T02:08:58.425572"}], "EXECUTED"),
        ([{"id": 594226727, "state": "CANCELED", "date": "2018-09-12T21:27:25.241689"}], "EXECUTED"),
        ([{"id": 615064591, "state": "CANCELED", "date": "2018-10-14T08:21:33.419441"}], "EXECUTED"),
    ],
)
def test_sort_by_date(start_list: Any, revers: Any) -> Any:
    try:
        assert sort_by_date(start_list) == revers
    except AssertionError:
        print("Некорректные данные")
